    ...
    <>
      ...
      <div className="flex h-[calc(100vh-104px)] min-h-0 flex-col gap-4 overflow-hidden">
        <div className="grid grid-cols-5 gap-4">
          ...
          <Stat icon={Bell} label="경고" value={String(distribution[1].value + distribution[2].value)} sub={`${(((distribution[1].value + distribution[2].value) / PANEL_DATA.length) * 100).toFixed(1)}%`} color="text-red-500" />
          ...
          <aside className="min-h-0 overflow-hidden">
            <section className="flex h-full min-h-0 flex-col rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
                {demoAlarms.concat(demoAlarms).map((alarm, index) => (
