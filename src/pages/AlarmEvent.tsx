      ...
      <div className="grid h-[calc(100vh-104px)] min-h-0 grid-rows-[96px_56px_minmax(0,1fr)] gap-4 overflow-hidden">
        ...
        <section className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-4 shadow-sm">
          <div className="relative flex-1">
            ...
            <input className="h-10 w-full rounded-lg border border-slate-200 pl-10 pr-3 text-sm outline-none focus:border-blue-500" placeholder="Search alarm, unit, equipment..." />
          </div>
          <select className="h-10 rounded-lg border border-slate-200 px-3 text-sm"><option>All severity</option><option>KEY-ALERT</option><option>WARNING</option></select>
          <select className="h-10 rounded-lg border border-slate-200 px-3 text-sm"><option>All status</option><option>Unresolved</option><option>Checking</option><option>Resolved</option></select>
          <button className="flex h-10 items-center gap-2 rounded-lg bg-blue-600 px-5 text-sm font-bold text-white"><Filter className="h-4 w-4" />Apply</button>
        </section>
        <section className="grid min-h-0 grid-cols-[1fr_320px] gap-4 overflow-hidden">
          <div className="overflow-auto rounded-lg border border-slate-200 bg-white shadow-sm">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {demoAlarms.concat(demoAlarms, demoAlarms).map((alarm, index) => (
          ...
          <aside className="space-y-4 overflow-hidden">
            <section className="rounded-lg border border-red-100 bg-red-50 p-5 text-red-700 shadow-sm">
              ...
              {demoAlarms.slice(0, 3).map((alarm) => (
                <button key={alarm.unitId} className="mb-3 w-full rounded-lg border border-red-100 bg-white p-3 text-left shadow-sm">
