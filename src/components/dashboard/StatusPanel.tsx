...
export function StatusPanel({ operations = [] }: Props) {
  ...
  return (
    <section className="min-h-0 overflow-hidden rounded-xl border-2 border-sky-200 bg-white shadow-sm">
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_74px_54px_36px] border-b border-slate-100 bg-slate-50 px-3 py-1.5 text-[9px] font-black uppercase tracking-wide text-slate-400">
        <span>기기번호</span>
        <span>키상태</span>
      ...
      <div className="max-h-[calc(100%-58px)] overflow-y-auto">
        {items.map((operation, index) => (
          <div key={operation.id} className={`grid grid-cols-[minmax(0,1fr)_74px_54px_36px] items-center gap-2 border-b border-slate-100 px-3 py-2 text-[10px] last:border-b-0 ${index % 2 === 1 ? 'bg-slate-50/70' : 'bg-white'}`}>
            ...
            <div className="text-center"><StatusBadge status={operation.status} /></div>
            <button type="button" className="rounded-md border border-blue-100 bg-blue-50 px-1.5 py-1 text-[9px] font-black text-blue-600">상세</button>
          </div>
        ))}
      </div>
    </section>
